...
class AdbActionBot:
    def __init__(self, adb : AdbController, vision : SudokuVision, pause_event: threading.Event, stop_event: threading.Event, positions: dict | None = None):
        self.adb = adb
        self.vision = vision
    ...
    def fill_result_relative(self, original_board, solved_board):
        ...
        if not self.positions:
            logger.warning("⚠️ 錯誤：行動部尚未取得座標地圖！")
            return
        ...
        for r in range(9):
            for c in range(9):
                ...
                # A. 點格子
                cx, cy = self.positions["cells"][(r, c)]
                self.adb.tap(cx, cy, offset=3, delay=(0.04, 0.08))

                # B. 點數字
                if val in self.positions["buttons"]:
                    bx, by = self.positions["buttons"][val]
                    self.adb.tap(bx, by, offset=3, delay=(0.04, 0.08))
                else:
                    logger.warning(f"⚠️ 找不到數字 {val} 的按鈕座標！")
    ...
    def click_target(self, img_name, timeout=30, threshold=0.8, wait_time = 0):  #等待並點擊
        ...
        while True:
            ...
            if elapsed_time > timeout:
                # 時間到了還沒找到
                if timeout > 0:
                    logger.warning(f"   等待超時 ({timeout}s)，未發現 {img_name}")
                return False
